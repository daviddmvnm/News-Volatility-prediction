# --------------------------------------------------------------
# Gold GPU Script 2 — GPU1
# Automatically processes ODD-index silver files only
# --------------------------------------------------------------
...
GOLD_DIR = FILE_DIR / "gold_parquet_gpu1"
GOLD_DIR.mkdir(exist_ok=True)

print(">>> GPU1 script started")
...
silver_files = all_files[1::2]   # ODD INDEX ONLY

print(f"GPU1 handling {len(silver_files)} silver files")
...
ner_pipe = pipeline("ner", model=ner_model, tokenizer=ner_tok, device=1, aggregation_strategy="simple")
...
def ner_counts(t):
    ents = ner_pipe(t[:1500])  # limit for speed
...
# ---------------- MAIN LOOP ----------------
batch_idx = 0
print("Starting GPU1 half-pipeline...")

for pdf in tqdm(batches(), desc="GPU1 batches"):
